...
in {
  ...
  scripts = {
    ...
    run-translator-proxy.exec = ''
      env RUST_LOG=translator_sv2=debug cargo -C roles/translator -Z unstable-options run -- \
    ...
    run-minerd.exec = "minerd -a sha256d -o stratum+tcp://127.0.0.1:34255 -D -P";
  };
